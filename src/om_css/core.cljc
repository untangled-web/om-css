...
(defmethod aug/defui-augmentation ::WithCssXForm
  [{:keys [defui/ui-name]} ast params]
  (-> ast
    (aug/wrap-augment 'Object 'render
      (fn [params body]
        `(localize-classnames ~ui-name ~body)))
    (aug/wrap-augment 'CSS 'css
      (fn [params body]
        `(transform (walker #(= (namespace ::here) (namespace %)))
           #(local-kw ~ui-name %)
           ~body)))))
